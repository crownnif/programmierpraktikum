...
class MauMauRunde
{
    ...
    {
        Kartenstapel stapel = new Kartenstapel(new Skatspiel());
        KartenTripel tripel = new KartenTripel();
        
        while(!tripel.istVoll())
        {
            if(istBildKarte(stapel.zeigeObersteKarte()))
            {
                tripel.fuegeHinzu(stapel.obersteKarteZiehen());
            }
        }
    ...
    private boolean istBildKarte(Spielkarte karte)
    {
        boolean ret = (karte.rang().equals(Kartenrang.BUBE)) || (karte.rang().equals(Kartenrang.DAME)) || (karte.rang().equals(Kartenrang.KOENIG));
        return  ret;
    }
